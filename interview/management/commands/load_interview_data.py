...
class Command(BaseCommand):
    ...
    def _write(self, msg):
        ...
        try:
            ...
            self.stdout.write(msg.encode('utf-8', errors='replace').decode('utf-8', errors='replace'))
    ...
    def _load_questions(self, filepath, filename):
        ...
        with transaction.atomic():
            ...
            if created_count:
                self._write(self.style.SUCCESS(f'  {created_count}/{total} questions created'))
            else:
    ...
    def _load_jump_rules(self, filepath, filename):
        ...
        for rule_data in rules_data:
            try:
                from_q = Question.objects.get(id=rule_data['from_question'], module=module)
                to_q = None
        ...
        if created_count:
            self._write(self.style.SUCCESS(f'  {created_count}/{len(rules_data)} jump rules created'))
        else:
